...
// 抽象数据类型方法声明及重载编译系统默认方法声明:
	...
	GenListNode<ElemType>* GetHead() const
	{
	    return head;
	}
	int CountElem(const GenListNode<ElemType> *L)
	{
	    int count=0;
	    for(GenListNode<ElemType> *p=L->tLink;p!=NULL;p=p->tLink)
        {
            if(p->tag==LIST)
            {
                count=count+CountElem(p->hLink);
            }
            else
                count++;
        }
