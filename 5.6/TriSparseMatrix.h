...
// 稀疏矩阵三元组顺序表的函数成员：
	...
	TriSparseMatrix<ElemType> operator+(const TriSparseMatrix<ElemType> &b)
	{
	    int i=0,j=0,sum=0;
        ...
        while(i<num&&j<num)
        {
            if(triElems[i].row==b.triElems[j].row)//获取到的两个元素在同一行
            {
                if(triElems[i].col==b.triElems[j].col)//获取到的两个元素同行同列
                {
                    //在数组中添加一个三元组，让value=两元素之和
                    if(triElems[i].value+triElems[j].value==0)
                    {
                        i++;j++;continue;
                    }
                    temp.triElems[temp.num].row=triElems[i].row;
                    temp.triElems[temp.num].col=triElems[i].col;
                    temp.triElems[temp.num].value=triElems[i].value+b.triElems[j].value;
                    temp.num++;
                    i++;j++;
                }
                else if(triElems[i].col<triElems[j].col)
                    //这一列的这个元素在矩阵a中有，在矩阵b中无;保存下a中的那个元素
                {
                    temp.triElems[temp.num].row=triElems[i].row;
                    temp.triElems[temp.num].col=triElems[i].col;
                    temp.triElems[temp.num].value=triElems[i].value;
                    temp.num++;
                    i++;
                }
                else
                    //这一列的这个元素在矩阵b中有，矩阵a中无
                {
                    temp.triElems[temp.num].row=b.triElems[j].row;
                    temp.triElems[temp.num].col=b.triElems[j].col;
                    temp.triElems[temp.num].value=b.triElems[j].value;
                    temp.num++;
                    j++;
                }
            }
            else if(triElems[i].row<b.triElems[j].row)
                //获取到的下一个元素在a中行号比b中行号小，说明这个元素在a中，不在b中
            {
                temp.triElems[temp.num].row=triElems[i].row;
                temp.triElems[temp.num].col=triElems[i].col;
                temp.triElems[temp.num].value=triElems[i].value;
