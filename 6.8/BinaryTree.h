...
//  二叉树方法声明及重载编译系统默认方法声明:
    ...
    ////
    void Print(BinTreeNode<ElemType> *r) const//类似中序遍历
    {
        if(r!=NULL)
        {
            if((r->leftChild!=NULL)&&OperPrior(r->leftChild->data,r->data)==1)
                //当左孩子的操作符优先级更低，就在执行左孩子前加括号
            {
                cout<<'(';
                Print(r->leftChild);//对左孩子递归调用
                cout<<')';
            }
            else Print(r->leftChild);//不加括号直接对左孩子递归调用
            cout<<r->data;//输出根
            if((r->leftChild!=NULL)&&OperPrior(r->rightChild->data,r->data)==1)
                 //当右孩子的操作符优先级更低，就在执行右孩子前加括号
            {
                cout<<'(';
                Print(r->rightChild);
                cout<<')';
            }
            else Print(r->rightChild);
