...
// 求以r为根的二叉树的结点个数
    ...
    int Depth(const BinTreeNode<ElemType> *r) const;
    int Depth2(BinTreeNode<ElemType> *r);
    int LeafCount(const BinTreeNode<ElemType> *r) const;
    int LeafCount2(BinTreeNode<ElemType> *r);
public:
//  二叉树方法声明及重载编译系统默认方法声明:
    ...
    int LeafCount() const;
    int LeafCount2();
    void ExchangeChild(BinTreeNode<ElemType> *r);
    void ExchangeChild();
    void ExchangeChild2(BinTreeNode<ElemType> *r);
    void ExchangeChild2();//非递归
};
...
template <class ElemType>
int BinaryTree<ElemType>::Depth(const BinTreeNode<ElemType> *r) const
// 操作结果：用递归算法返回以r为根的二叉树的深度
{
	if(r == NULL)	// 空二叉树深度为0
		return 0;
	else
    {
		int ldepth, rdepth;
		ldepth = Depth(r->leftChild);
		rdepth = Depth(r->rightChild);
		return (ldepth > rdepth ? ldepth : rdepth) + 1;
        // 非空二叉树深度为左右子树的高的最大值再加1
	}
}
template <class ElemType>
int BinaryTree<ElemType>::Depth() const
// 操作结果：返回二叉树的深度
{
	return Depth(root);
}
...
template <class ElemType>
int BinaryTree<ElemType>::Depth2(BinTreeNode<ElemType> *r)//非递归
{
      if (r==NULL)  //空二叉树叶子数为0
      {
            return 0;
      ...
      int _depth=0;
      LinkStack<BinTreeNode<ElemType>* > s;//定义一个栈
      BinTreeNode<ElemType> *temp = NULL;
      BinTreeNode<ElemType> *p=r;//指向结点的指针
      while(p||!s.IsEmpty())//当指针非空或者栈非空
      {
            while(p)//当指针非空，就入栈
            {
                  s.Push(p);
                  if(_depth<s.GetLength())
                        _depth=s.GetLength()+1;
                  p=p->leftChild;
            }
            if(!s.IsEmpty())//否则，弹出
            {
...
template <class ElemType>
int BinaryTree<ElemType>::Depth2()
{
	return Depth2(root);
}
...
int BinaryTree<ElemType>::LeafCount(const BinTreeNode<ElemType> *r) const
{
      ...
      if (NULL == r->leftChild && NULL == r->rightChild)
      {
            return 1;  //是叶子时，返回1
      }
...
template <class ElemType>
int BinaryTree<ElemType>::LeafCount2(BinTreeNode<ElemType> *r)//非递归
{
      if (r==NULL)  //空二叉树叶子数为0
      {
            return 0;
...
template <class ElemType>
int BinaryTree<ElemType>::LeafCount2()
{
...
template <class ElemType>
void BinaryTree<ElemType>::ExchangeChild(BinTreeNode<ElemType> *r)//交换左右子树
{
     BinTreeNode<ElemType> *temp = NULL;
     if (r==NULL)  //空二叉树
     {
     ...
     if(r->leftChild)
          ExchangeChild(r->leftChild);
     if(r->rightChild)
          ExchangeChild(r->rightChild);
}
template <class ElemType>
void BinaryTree<ElemType>::ExchangeChild()
{
	ExchangeChild(root);
}
...
template <class ElemType>
void BinaryTree<ElemType>::ExchangeChild2(BinTreeNode<ElemType> *r)
{
      LinkStack<BinTreeNode<ElemType>* > s;//定义一个栈
      BinTreeNode<ElemType> *temp = NULL;
      BinTreeNode<ElemType> *p=r;//指向结点的指针
...
template <class ElemType>
void BinaryTree<ElemType>::ExchangeChild2()
{
	ExchangeChild2(root);
}
