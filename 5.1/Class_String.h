...
public:
    ...
    String(const int n,const char &c);           //构造函数，实现初始化string s(3,'a');
    String(const String &s);                    //拷贝构造函数，实现初始化string a(b);深拷贝
    //析构函数
    virtual ~String();                          //析构函数
    //普通成员函数
    int length() const;                         //返回字符串长度
    int KMP_find(const String &b,int &p);
    void Failure(const String &b,int *f);
    String & Replace(const String &t,const String &v);
    bool empty() const;                         //判断字符串是否为空
    String& erase();                            //清空字符串
    String& swap(String &s);                    //交换两个字符串
    //运算符重载
    char& operator[](int index) const;     //重载[]运算符，实现s[i]下标访问
