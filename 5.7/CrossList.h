...
// 稀疏矩阵十字链的方法声明:
    ...
    CrossList<ElemType> operator *(const CrossList<ElemType> &b)
    {
        ...
        CrossList<ElemType> temp(rows,b.cols);//用a的行数和b的列数构造矩阵
        CrossNode<ElemType> *p=NULL,*q=NULL;//定义两个用于遍历a和b矩阵元素的指针

        for(int i=0;i<rows;i++)
        {
            for(int j=0;j<b.cols;j++)//对于新矩阵进行遍历
            {
                ElemType sum=0;
                p=rowHead[i];//取出a的第i这一行
                q=b.colHead[j];
                while(p!=NULL&&q!=NULL)
                {
                    if(p->triElem.col<q->triElem.row)//当位置不匹配时，移动至位置匹配
                    {
                        p=p->right;
                        continue;
                    }
                    ...
                    sum+=p->triElem.value*q->triElem.value;
                    p=p->right;q=q->down;
                }
                if(sum!=0)
                {
                    temp.SetElem(i,j,sum);
                }
            }
