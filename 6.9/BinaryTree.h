...
//  二叉树方法声明及重载编译系统默认方法声明:
    ...
    {
        //采用层序遍历方法
        LinkQueue<BinTreeNode<ElemType>* > q;//定义一个队列
        BinTreeNode<ElemType> *p;
        int n,m,max;//m表示这一层的宽度,max表示当前为止获取到的最大宽度,n是临时变量来保存当前这一层宽度的值
        if(root!=NULL)
        {
            q.EnQueue(root);//把根入队
            n=1;m=1;max=1;
        }
        while(!q.IsEmpty())//当队列非空
        {
            m=q.GetLength();//这个时候队列的长度就等于这一层的宽度
            n=m;//让临时变量保存宽度
            if(max<m) max=m;//让max取m的最大值
            for(int i=0;i<n;i++)
                //这一层有n个结点，所以要执行n次，把这n个结点全部从队列中移除，并把这n个结点的孩子放进队列
            {
                q.DelQueue(p);
                if(p->leftChild!=NULL) q.EnQueue(p->leftChild);//把左孩子放进队列
